...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if len(menus) == 0:
            ...
            init_menu(menumodel)
            menumodel = Menu.objects.create(name='菜單管理', code='menu', curl='/menu', icon='menu', sequence=40, type=2, parent=sysmenu)
            init_menu(menumodel)
            menumodel = Menu.objects.create(name='圖標管理', code='icon', curl='/icon', icon='icon', sequence=50, type=2, parent=sysmenu)
            init_menu(menumodel)
            ...
            menumodel = Menu.objects.create(name='審計日誌', code='audit', curl='/audit', icon='audit', sequence=10, type=2, parent=toolmenu)
            init_menu(menumodel)
            menumodel = Menu.objects.create(name='測試頁面', code='test', curl='/test', icon='list', sequence=20, type=2, parent=toolmenu)
            init_menu(menumodel)
            ...
            menumodel = Menu.objects.create(name='郵箱通知', code='mail', curl='/mail', icon='mail', sequence=10, type=2, parent=noticemenu)
            init_menu(menumodel)
