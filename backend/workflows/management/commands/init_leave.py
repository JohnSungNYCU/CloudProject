...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        ## 工作流类型
        it_type = WorkflowType.objects.create(name='技術', code='it', order_id=2)

        ## 工作流
        deploy_wf = Workflow.objects.create(name='發布單', type=it_type, ticket_sn_prefix='deploy')
        ...
        CustomField.objects.create(field_name="工號", order_id=4, field_attribute=True, field_type=2, field_key="id",
                                   workflow=deploy_wf)
        # 建立扩展字段
        c1 = CustomField.objects.create(field_name="發布時間", order_id=10, field_type=6, field_key="start_time",
                                        workflow=deploy_wf)
        c2 = CustomField.objects.create(field_name="發布項目", order_id=30, field_type=9, field_key="type",
                                        field_choice='{"1":"前端", "2":"後端"}', workflow=deploy_wf)
        c3 = CustomField.objects.create(field_name="發布內容", order_id=50, field_type=8, field_key="memo",
                                        workflow=deploy_wf)
        c4 = CustomField.objects.create(field_name="領導審批", order_id=60, field_type=9, field_key="leader_radio",
                                        field_choice='{"1":"同意", "2":"不同意"}', workflow=deploy_wf)
        c5 = CustomField.objects.create(field_name="運維執行", order_id=80, field_type=9, field_key="ops_radio",
